...
```bash
docker pull sameersbn/gitlab:18.5.2
```
...
docker run --name gitlab -d [OPTIONS] \
    sameersbn/gitlab:18.5.2 app:sanitize
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:18.5.2 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:18.5.2 app:rake db:setup
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:18.5.2 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:18.5.2 app:rake gitlab:backup:restore BACKUP=1515629493_2020_12_06_13.0.6
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:18.5.2 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:18.5.2 app:rake gitlab:import:repos
```
...
> **Note**
>
> Upgrading to `sameersbn/gitlab:18.5.2` from `sameersbn/gitlab:7.x.x` can cause issues. It is therefore required that you first upgrade to `sameersbn/gitlab:8.0.5-1` before upgrading to `sameersbn/gitlab:8.1.0` or higher.
...
```bash
docker run --name gitlab -d [OPTIONS] sameersbn/gitlab:18.5.2
```
